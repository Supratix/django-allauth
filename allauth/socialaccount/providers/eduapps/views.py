...
from .eduapps_session import add_eduapps_session, persist_eduapps_session
from .client import EduappsOAuth2Client
from .provider import EduappsProvider


class EduappsOAuth2Adapter(OAuth2Adapter):
    client_cls = EduappsOAuth2Client
    provider_id = EduappsProvider.id
    access_token_url = "https://eduappsid.eduapps.com/auth/token"
    authorize_url = "https://eduappsid.eduapps.com/auth/authorize"
    public_key_url = "https://eduappsid.eduapps.com/auth/keys"

    def _get_eduapps_public_key(self, kid):
        response = requests.get(self.public_key_url)
        response.raise_for_status()
        try:
            data = response.json()
        except json.JSONDecodeError as e:
            raise OAuth2Error("Error retrieving eduapps public key.") from e
    ...
    def get_public_key(self, id_token):
        ...
        eduapps_public_key = self._get_eduapps_public_key(kid=kid)

        public_key = jwt.algorithms.RSAAlgorithm.from_jwk(
            json.dumps(eduapps_public_key)
        )
        return public_key
    ...
    def get_verified_identity_data(self, id_token):
        ...
        try:
            ...
            identity_data = jwt.decode(
                ...
                issuer="https://eduappsid.eduapps.com",
            )
    ...
    def parse_token(self, data):
        ...
        # access_tokens, and user info from the eduapps post.
    ...
    def complete_login(self, request, app, token, **kwargs):
        ...
        # We can safely remove the eduapps login session now
        # Note: The cookie will remain, but it's set to delete on browser close
        try:
            request.eduapps_login_session.delete()
    ...
    def get_user_scope_data(self, request):
        user_scope_data = request.eduapps_login_session.get("user", "")
        try:
    ...
    def get_access_token_data(self, request, app, client):
        """ We need to gather the info from the eduapps specific login """
        add_eduapps_session(request)
        ...
        return {
            ...
            **self.get_user_scope_data(request),
            "id_token": request.eduapps_login_session.get("id_token"),
        }


@csrf_exempt
def eduapps_post_callback(request, finish_endpoint_name="eduapps_finish_callback"):
    """
    eduapps uses a `form_post` response type, which due to
    CORS/Samesite-cookie rules means this request cannot access
    the request since the session cookie is unavailable.

    We work around this by storing the eduapps response in a
    ...
    # Add other params to the eduapps_login_session
    keys_to_save_to_session = ["user", "id_token"]
    for key in keys_to_save_to_session:
        request.eduapps_login_session[key] = get_request_param(request, key, "")

    url = request.build_absolute_uri(reverse(finish_endpoint_name))
    response = HttpResponseRedirect(
        "{url}?{query}".format(url=url, query=urlencode(url_params))
    )
    persist_eduapps_session(request, response)
    return response


oauth2_login = OAuth2LoginView.adapter_view(EduappsOAuth2Adapter)
oauth2_callback = eduapps_post_callback
oauth2_finish_login = OAuth2CallbackView.adapter_view(EduappsOAuth2Adapter)
