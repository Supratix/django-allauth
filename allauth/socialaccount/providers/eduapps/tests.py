...
from .eduapps_session import EDUAPPS_SESSION_COOKIE_NAME
from .provider import eduappsProvider


# Generated on https://mkjwk.org/, used to sign and verify the eduapps id_token
TESTING_JWT_KEYSET = {
    "p": (
...
# Mocked version of the test data from https://eduappsid.eduapps.com/auth/keys
...
def sign_id_token(payload):
    """
    Sign a payload as eduapps normally would for the id_token.
    """
...
class eduappsTests(OAuth2TestsMixin, TestCase):
    provider_id = eduappsProvider.id

    def get_eduapps_id_token_payload(self):
        now = datetime.utcnow()
        return {
            "iss": "https://eduappsid.eduapps.com",
            "aud": "app123id",  # Matches `setup_app`
            "exp": now + timedelta(hours=1),
            "iat": now,
            "sub": "000313.c9720f41e9434e18987a.1218",
            "at_hash": "CkaUPjk4MJinaAq6Z0tGUA",
            "email": "test@privaterelay.eduappsid.com",
            "email_verified": "true",
    ...
    def get_login_response_json(self, with_refresh_token=True):
        """
        `with_refresh_token` is not optional for eduapps, so it's ignored.
        """
        id_token = sign_id_token(self.get_eduapps_id_token_payload())
    ...
    def get_mocked_response(self):
        """
        eduapps is unusual in that the `id_token` contains all the user info
    ...
    def get_complete_parameters(self, auth_request_params):
        """
        Add eduapps specific response parameters which they include in the
        form_post response.
        ...
        params.update(
            {
                ...
                "user": json.dumps(
                    {
                        "email": "private@eduappsid.eduapps.com",
    ...
    def login(self, resp_mock, process="login", with_refresh_token=True):
        ...
        with mocked_response(
            ...
            assert reverse("eduapps_finish_callback") in resp.url
    ...
    def test_authentication_error(self):
        """ Override base test because eduapps posts errors """
    ...
    def test_eduapps_finish(self):
        ...
        # Check have cookie containing eduapps session
        self.assertTrue(EDUAPPS_SESSION_COOKIE_NAME in self.client.cookies)

        # Session should have been cleared
        eduapps_session_cookie = self.client.cookies.get(EDUAPPS_SESSION_COOKIE_NAME)
        engine = import_module(settings.SESSION_ENGINE)
        SessionStore = engine.SessionStore
        eduapps_login_session = SessionStore(eduapps_session_cookie.value)
        self.assertEqual(len(eduapps_login_session.keys()), 0)

        # Check cookie path was correctly set
        self.assertEqual(eduapps_session_cookie.get("path"), finish_url)
