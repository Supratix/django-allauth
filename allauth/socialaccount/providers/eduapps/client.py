...
class EduAppsOAuth2Client(OAuth2Client):
    """
    Custom client because `Sign In With EduApps`:
        * requires `response_mode` field in redirect_url
        * requires special `client_secret` as JWT
    """

    def generate_client_secret(self):
        """Create a JWT signed with an EduApps provided private key"""
        now = datetime.utcnow()
        claims = {
            "iss": self.key,
            "aud": "https://eduapps.eduapps.com",
